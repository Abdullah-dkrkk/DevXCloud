...
<p style="margin:5px 0 0;font-size:14px;opacity:0.9;">{{ $ticket->ticket_number }} was closed by {{ $closedBy?->name ?? 'System' }}</p>
</td></tr>
...
<tr><td style="padding:10px;"><strong>Closed By</strong></td><td style="padding:10px;">{{ $closedBy?->name ?? 'Auto-close System' }}</td></tr>
<tr style="background:#f9fafb;"><td style="padding:10px;"><strong>Closed At</strong></td><td style="padding:10px;">{{ now()->format('F j, Y, g:i a') }}</td></tr>
</table>
</td></tr>
@else
<tr><td style="background:#0176d3;color:#ffffff;padding:20px 30px;">
<h2 style="margin:0;font-size:22px;">Your Ticket Has Been Closed</h2>
...
<p style="font-size:14px;color:#555;line-height:1.6;">
<a href="{{ url('/') }}" style="display:inline-block;background:#0176d3;color:#fff;padding:10px 24px;border-radius:6px;text-decoration:none;font-size:14px;font-weight:bold;">Open New Ticket</a>
</p>
</td></tr>
@endif
