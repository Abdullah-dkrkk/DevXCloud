<div class="flex flex-col md:flex-row h-full gap-0 md:gap-4 overflow-hidden" x-data="{ activeTab: 'open', pollId: null, mobileView: 'list' }" x-init="pollId = setInterval(() => { $wire.$refresh(); }, 2000)">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col md:flex md:w-2/5" x-bind:class="{'hidden': $wire.selectedTicketId && mobileView === 'chat'}">
        <style>
            ...
            @media (max-width: 360px) {
                .action-btns { display: grid !important; grid-template-columns: 1fr 1fr; gap: 4px; }
                .action-btns > button { width: 100%; justify-content: center; }
                .action-btns > button:nth-child(3) { grid-column: 1 / -1; }
            }
        </style>
                        <div class="p-3 md:p-4 border-b bg-gray-50 flex items-center justify-between">
                            ...
                            <div class="flex gap-1.5">
                                <button x-on:click="activeTab = 'open'"
                                        ...
                                        class="px-2.5 py-1.5 md:px-3.5 md:py-1.5 rounded-lg text-xs font-medium transition">Open</button>
                                <button x-on:click="activeTab = 'mine'"
                                        ...
                                        class="px-2.5 py-1.5 md:px-3.5 md:py-1.5 rounded-lg text-xs font-medium transition">Mine</button>
                                <button x-on:click="activeTab = 'closed'"
                                        ...
                                        class="px-2.5 py-1.5 md:px-3.5 md:py-1.5 rounded-lg text-xs font-medium transition">Closed</button>
                            </div>
                        </div>
        <div class="flex-1 overflow-y-auto agent-ticket-list flex flex-col">
            <template x-for="ticket in $wire.tickets.filter(t => activeTab === 'open' ? (t.status !== 'closed' && !t.assigned_to) : activeTab === 'mine' ? (t.status !== 'closed' && t.assigned_to === {{ auth()->id() }}) : t.status === 'closed')" :key="ticket.id">
                <div wire:click="selectTicket(ticket.id)" @click="mobileView = 'chat'; $wire.messages = []; $wire.selectedTicketId = null;"
                     :class="{
                     ...
                     class="p-3 md:p-4 border-l-4 cursor-pointer transition">
                    <div class="flex items-start gap-3">
                        <div class="w-9 h-9 md:w-10 md:h-10 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0 mt-0.5"
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <button x-show="!ticket.assigned_to && activeTab === 'open'"
                                    ...
                                    class="mt-2.5 text-xs bg-green-600 text-white px-3 py-1 rounded-lg hover:bg-green-700 transition font-medium inline-flex items-center">
                                + Claim
                            </button>
                        </div>
                    </div>
            ...
            <div x-show="$wire.tickets.filter(t => activeTab === 'open' ? (t.status !== 'closed' && !t.assigned_to) : activeTab === 'mine' ? (t.status !== 'closed' && t.assigned_to === {{ auth()->id() }}) : t.status === 'closed').length === 0"
                 class="flex-1 flex items-center justify-center p-4 md:p-8 text-center">
                <div>
    ...
    <div class="flex-1 min-w-0 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col md:flex relative" x-bind:class="{'hidden': mobileView === 'list' || !$wire.selectedTicketId}">
        <template x-if="$wire.selectedTicketId">
            <div class="flex flex-col flex-1 min-h-0">
                <div class="p-3 md:p-3.5 border-b bg-gray-50 flex items-center gap-2 md:gap-3">
                    <button @click="mobileView = 'list'" class="md:hidden flex-shrink-0 p-1 -ml-1 rounded-lg hover:bg-gray-200 transition" aria-label="Back to tickets">
                        <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                    </button>
                    <div class="w-9 h-9 md:w-10 md:h-10 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0"
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center gap-2 flex-wrap">
                            <span class="font-medium text-sm text-gray-800 truncate" x-text="$wire.selectedTicket?.name"></span>
                            <span class="text-xs px-2 py-0.5 rounded font-medium"
                                  :class="{
                        ...
                        <div class="text-xs text-gray-500 flex items-center gap-2 mt-0.5 min-w-0">
                            <span class="truncate" x-text="$wire.selectedTicket?.email"></span>
                            <span class="text-gray-300 flex-shrink-0">|</span>
                            <span class="font-mono text-gray-400 flex-shrink-0" x-text="$wire.selectedTicket?.ticket_number"></span>
                        </div>
                    </div>
                </div>

                <div class="flex-1 overflow-y-auto p-3 md:p-4 flex flex-col chat-messages-scroll" id="messages-container">
                    <template x-for="msg in $wire.messages" :key="msg.id">
                        <div>
                        <template x-if="msg.sender_type === 'system'">
                            <div class="flex flex-col items-center my-2">
                                <div class="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 md:px-4 md:py-2.5 text-center w-[85%] md:w-[75%]">
                        ...
                        <template x-if="msg.sender_type !== 'system'">
                            <div class="flex gap-2.5 mb-2.5" :class="msg.sender_type === 'agent' ? 'flex-row-reverse self-end' : 'self-start'">
                                <template x-if="msg.sender_type === 'bot'">
                                    <div class="w-7 h-7 md:w-[34px] md:h-[34px] mt-1 rounded-full flex items-center justify-center flex-shrink-0 bg-gray-100 text-gray-500">
                                ...
                                <template x-if="msg.sender_type !== 'bot'">
                                    <div :class="msg.sender_type === 'agent' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-500'"
                                         class="w-7 h-7 md:w-[34px] md:h-[34px] mt-1 rounded-full flex items-center justify-center flex-shrink-0">
                                ...
                                <div class="flex flex-col" :class="msg.sender_type === 'agent' ? 'items-end' : 'items-start'">
                                    <div :class="msg.sender_type === 'agent' ? 'bg-[#0176D3] text-white break-words' : 'bg-gray-50 text-gray-800 shadow-sm border border-gray-200 break-words'"
                                         :style="msg.sender_type === 'agent' ? 'border-radius:1rem;border-top-right-radius:0' : 'border-radius:1rem;border-top-left-radius:0'"
                                         class="px-3 py-2 md:px-3.5 md:py-2.5 text-sm leading-relaxed overflow-hidden">
                    ...
                    <div x-show="$wire.userTyping" class="flex gap-2.5 mb-2.5 self-start">
                        <div class="w-7 h-7 md:w-[34px] md:h-[34px] mt-1 rounded-full flex items-center justify-center bg-white border border-gray-200 flex-shrink-0">
                        ...
                        <div class="flex flex-col">
                            <div class="bg-gray-50 rounded-t-2xl rounded-br-2xl shadow-sm border border-gray-200 px-3 py-2.5 md:px-3.5 md:py-3">
                ...
                <div x-show="$wire.showForceConfirm" class="px-3 py-3 md:px-4 md:py-4">
                    <div class="bg-red-50 border border-red-200 rounded-lg p-3 flex flex-col sm:flex-row items-start sm:items-center gap-3">
                ...
                <div x-show="$wire.selectedTicket?.status !== 'closed'" class="px-3 py-2.5 md:px-4 border-t bg-gray-50">
                    <div class="action-btns flex gap-1 md:gap-2">
                        <button wire:click="sendReminder()"
                                class="flex items-center gap-1.5 text-xs px-2 py-1.5 md:px-3 md:py-1.5 rounded-lg bg-amber-50 text-amber-700 border border-amber-200 hover:bg-amber-100 transition font-medium">
                            <svg class="hidden md:inline w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>
                            Remind
                        </button>
                        <button wire:click="requestClose()"
                                class="flex items-center gap-1.5 text-xs px-2 py-1.5 md:px-3 md:py-1.5 rounded-lg bg-green-50 text-green-700 border border-green-200 hover:bg-green-100 transition font-medium">
                            <svg class="hidden md:inline w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            Close with Buttons
                        </button>
                        <button wire:click="confirmForceClose()"
                                class="flex items-center gap-1.5 text-xs px-2 py-1.5 md:px-3 md:py-1.5 rounded-lg bg-red-50 text-red-700 border border-red-200 hover:bg-red-100 transition font-medium">
                            <svg class="hidden md:inline w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                            Force Close
                        </button>
                    </div>
                </div>

                <div x-show="$wire.selectedTicket?.status === 'closed'" class="p-3 md:p-3.5 border-t bg-gray-50">
        ...
        <div wire:key="agent-input-bar" x-data="{ showInput: false, localMsg: '', sending: false }" x-on:input-visibility.window="showInput = $event.detail.status !== 'closed'" x-show="showInput" class="border-t">
            <div class="flex gap-2 p-2.5 md:gap-2.5 md:p-3.5">
                <input type="text" x-model="localMsg"
                       ...
                       class="flex-1 border border-gray-300 rounded-lg px-3 py-2.5 md:px-4 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
                ...
                <button type="button" x-on:click.prevent="if (!sending && localMsg.trim()) { sending = true; $wire.sendMessage(localMsg).then(function(){ sending = false; localMsg = ''; }).catch(function(){ sending = false; localMsg = ''; }); }"
                        :disabled="sending"
                        class="bg-blue-600 text-white px-4 py-2.5 md:px-5 rounded-lg text-sm font-medium hover:bg-blue-700 transition shadow-sm disabled:opacity-70 disabled:cursor-not-allowed min-w-[60px] md:min-w-[72px] flex items-center justify-center gap-1.5">
